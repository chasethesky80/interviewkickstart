...
public class FindPeakElementInAnArray {
    ...
    public static void main(String[] args) {
        ...
        System.out.println("Find peak element in an array "+ findPeakElement(new int[]{ 1, 5, 1, 2, 1 }));
    }
    private static int findPeakElement(int[] nums) {
        ...
        while (low <= high) {
            ...
            if (nums[mid-1] < nums[mid] && nums[mid] > nums[mid + 1]) {
                return mid;
            } else if (nums[mid-1] < nums[mid]) { // INCREASING CURVE - PEAK ON RIGHT
                low = mid + 1;
            } else {
                high = mid - 1; // ELSE PEAK ON LEFT
            }
        }
        return low;
    }
